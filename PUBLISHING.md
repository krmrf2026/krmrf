...
## 13. Ссылки на источники при публикации

Внешние ссылки ставятся прямо в HTML публикации — в блоке «Источники и ограничения» или непосредственно рядом с соответствующим утверждением.

Отдельного реестра и механизма сохранения копий в проекте нет. После добавления ссылок достаточно запустить:

```bash
npm run check
```

Проверка контролирует синтаксис HTML и URL, но не скачивает сторонние страницы и не создаёт архивные копии.
