...
try { searchTermEntries = decodeSearchTerms(searchPayload); } catch {}
...
for (const query of queries) {
  ...
  const inTerms = searchTermEntries.some(([term]) => term.includes(query));
  if (!inMetadata && !inTerms) errors.push(`Поиск: контрольный термин «${query}» отсутствует.`);
}
if (searchPayload.version !== 3 || searchDocuments.length !== pages.length) errors.push('Поиск: индекс v3 не согласован с pages.json.');
