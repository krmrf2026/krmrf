...
У него нет CMS, базы данных и панели администратора. Каждая статья — обычный `index.html`. Файл `data/pages.json` — единственный каталог публикаций и их метаданных, а команды Node.js автоматически обновляют главную, разделы, архив, поиск, sitemap и данные версии.
...
Обычно не меняйте `dist/`, `data/search-index.json`, `data/site.json`, `sitemap.xml`, карточки на главной и в разделах. Их создаёт `npm run build` или `npm run qa`.

Старые URL перечислены в служебном `_redirects`, но GitHub Pages этот файл не исполняет. Во время `npm run export:public` из реестра автоматически создаются статические страницы переноса только внутри `dist/`. В исходниках они не хранятся.
...
Публикация выполняется workflow `.github/workflows/pages.yml` после попадания проверенной версии в `main`. Workflow сам создаёт `dist/` и передаёт GitHub Pages только эту папку. Нельзя публиковать корень репозитория или смешивать отдельные файлы разных выпусков.

Один раз в настройках репозитория нужно выбрать **Settings → Pages → Source: GitHub Actions**, проверить домен `krmrf.ru` и включённый HTTPS. Файл `CNAME` сохраняет домен в артефакте, но не заменяет настройку домена в интерфейсе GitHub.

До 29 июля 2027 года действует [техническая заморозка](TECHNICAL_FREEZE.md): разрешены публикации, редактура, обновление карты и исправление реальных поломок; новые платформы и функции не добавляются.
