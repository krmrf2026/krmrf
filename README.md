...
## 17. Как ставить ссылки на источники

Обычные внешние ссылки остаются непосредственно в HTML публикации — обычно в блоке «Источники и ограничения» или рядом с утверждением, которое они подтверждают.

Отдельного реестра и механизма архивирования внешних страниц в проекте нет. `npm run check` проверяет структуру сайта и синтаксис внешних URL.

Правила простые:

- ставь ссылку на конкретный документ или публикацию, а не только на главную страницу сайта;
- называй источник понятным текстом;
- для ссылок, открывающихся в новой вкладке, используй `rel="noopener"`;
- не считай доступность стороннего сайта частью работоспособности KRM РФ.
